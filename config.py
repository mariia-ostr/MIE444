...
robot_width = 8                 # Robot width in inches
robot_height = 6
...
num_edges = 20
radius = robot_width / 2  # Radius for the circle
# Create a list to hold the vertices
robot_outline = []

# Calculate the vertices
for i in range(num_edges):
    angle = 2 * math.pi * i / num_edges  # Convert to radians
    x = radius * math.cos(angle)  # X coordinate
    y = radius * math.sin(angle)  # Y coordinate
    robot_outline.append(pm.Vector2(x, y))
...
u0_info = {
    'id': 'u0',
    'position': [-3.117672, -1.8], #[-4.3301,-2.5]
    'height': 2,
    'rotation': 120,
    'error': 0.02,
    'outline': [
